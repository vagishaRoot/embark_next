...
//   const location = useLocation();
  const blogId = router.replace('/blogs/','')
      ...
      : <div className="flex items-center justify-center mb-20 mt-10  ">
        {Object.keys(blogData).length
          ?  <div className="flex flex-col items-center w-[70%] gap-y-[20px] 768-425:w-[85%] max-425:w-[90%]">
              {blogData.image && (
                <img src={blogData.image} alt="..." className=" object-contain  w-[70%] h-60  mx-auto max-425:w-[250px] " />
              )}
              {blogData.title && (
                <h1 className="text-xl font-bold w-full capitalize ">{blogData.title}</h1>
              )}
              {(blogData.link && blogData.subtitle) ? <button onClick={goToPage} className="bg-blue-500 text-white px-[20px] py-[10px]">{blogData.subtitle}</button> : <></>}
