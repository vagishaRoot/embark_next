  ...
  const buyNow = (product = undefined) => {
    ...
    if (!!product) {
      ...
      cartDetails.forEach((v) => {
        ...
        objs["quantity"] = 1;
        objs["price"] = !!v.productId.discount
          ? parseFloat(
              parseFloat(v.productId.price) -
                (parseFloat(v.productId.price) * v.productId.discount) / 100
            ).toFixed(2)
          : parseFloat(v.productId.price).toFixed(2);
        arr.push(objs);
      });
    }
    const header = {
                      ...
                      >
                        ...
                        <div className="flex flex-col items-start h-full justify-between py-[10px]">
                          ...
                          <div className="flex gap-x-[10px]">
                            ...
                            >
                              {cartByIdLoader === idx.productId._id ? (
                                <Icons
                                  string="loading"
                                  width="24px"
                                  height="24px"
